...
function provides a tibble with data from the [Awesome Spectral Indices
project](https://github.com/awesome-spectral-indices/awesome-spectral-indices):
...
The first time
[`spectral_indices()`](https://docs.ropensci.org/rsi/reference/spectral_indices.md)
is called it will download the most up-to-date version of the spectral
indices JSON file, and then write the resulting table to a cache file in
`tools::R_user_dir("rsi")`. After that,
[`spectral_indices()`](https://docs.ropensci.org/rsi/reference/spectral_indices.md)
will only download a new file if the cache is older than 1 day, or if
the `update_cache` argument is `TRUE`, in order to provide the most
up-to-date data as quickly as possible. If offline,
[`spectral_indices()`](https://docs.ropensci.org/rsi/reference/spectral_indices.md)
will always fall back to the cache or, if no cache file exists, a
(possibly out-of-date) data file included in rsi itself.

Separately, the
[`get_stac_data()`](https://docs.ropensci.org/rsi/reference/get_stac_data.md)
function provides a generic interface for downloading composite images
from any accessible STAC catalog. For instance, we could download a
cloud-masked composite of Landsat’s visible layers using
[`get_stac_data()`](https://docs.ropensci.org/rsi/reference/get_stac_data.md)
and a few helper functions from rsi:
...
most of these arguments for you. For instance, that
[`get_stac_data()`](https://docs.ropensci.org/rsi/reference/get_stac_data.md)
...
(after
[`terra::stretch()`](https://rspatial.github.io/terra/reference/stretch.html)ing
...
function, well, calculates indices, using subsets of our
[`spectral_indices()`](https://docs.ropensci.org/rsi/reference/spectral_indices.md)
data frame:
